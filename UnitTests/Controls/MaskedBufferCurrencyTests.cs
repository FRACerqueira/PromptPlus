...
namespace PPlus.Tests.Controls
{
    ...
    public class MaskedBufferCurrencyTests : BaseTest
    {
        [Fact]
        internal void Should_have_accept_Load_validvalues_en_us()
        {
            // Given
            var maskedBuffer = new MaskedBuffer(new OptionsForMaskeditCurrency(new CultureInfo("en-US"), 10, 2,true, null));
        ...
        {
            // Given
            var maskedBuffer = new MaskedBuffer(new OptionsForMaskeditCurrency(new CultureInfo("pt-BR"), 10, 2,true, null));
            maskedBuffer.Load(maskedBuffer.RemoveMask("123,45", false));
            //then
            Assert.Equal("R$ 0.000.000.123,45", maskedBuffer.ToMasked());
            Assert.Equal(11, maskedBuffer.Position);
            Assert.Equal(12, maskedBuffer.Length);
            Assert.True(maskedBuffer.IsMaxInput);
            Assert.True(maskedBuffer.IsEnd);
        ...
        internal void Should_have_not_accept_Load_invalidvalues_pt_br()
        {
            ...
            maskedBuffer.Load(maskedBuffer.RemoveMask("12A.45", false));
            //then
            Assert.Equal("R$ 0.000.000.000,00", maskedBuffer.ToMasked());
            Assert.Equal(9, maskedBuffer.Position);
            Assert.Equal(12, maskedBuffer.Length);
            Assert.True(maskedBuffer.IsMaxInput);
            Assert.False(maskedBuffer.IsEnd);
        ...
        {
            public OptionsForMaskeditCurrency(CultureInfo culture, int ammountInteger, int ammountDecimal, bool acceptSignal, string defaultvalue) : base(false)
            {
