...
{
    internal class TableSelectControl<T> : BaseControl<ResultTableSelect<T>>, IControlTableSelect<T> where T : class
    {
        ...
        public TableSelectControl(IConsoleControl console, TableOptions<T> options) : base(console, options)
        {
            _options = options;
        }
        ...
        public override void InputTemplate(ScreenBuffer screenBuffer)
        {
            ...
            string answer = null;
            if (_options.SelectedTemplate != null && !_options.OptMinimalRender)
            {
            ...
            if (_tableviewport.startWrite != 0 || _tableviewport.endwrite != _totalTableLenWidth)
            {
                ...
                screenBuffer.AddBuffer($"{spc}{viewstartcol}~{viewendcol} / {tot}", _options.StyleContent(StyleControls.Pagination), true);
            }
        ...
        public override void FinishTemplate(ScreenBuffer screenBuffer, ResultTableSelect<T> result, bool aborted)
        {
            if (!aborted)
            {
        ...
        public override ResultPrompt<ResultTableSelect<T>> TryResult(CancellationToken cancellationToken)
        {
            var endinput = false;
            var abort = false;
            ...
            if (_localpaginator.Count == 0)
            {
                return new ResultPrompt<ResultTableSelect<T>>(ResultTableSelect<T>.NullResult(),abort, !endinput, notrender);
            }
            return new ResultPrompt<ResultTableSelect<T>>(
                new ResultTableSelect<T>(_currentrow, _currentcol,_localpaginator.SelectedItem.Value, GetValueColumn(_currentcol, _localpaginator.SelectedItem)), 
                abort, !endinput, notrender);
        }

        #region IControlTableSelct

        public IControlTableSelect<T> AutoSelect(bool value = true)
        {
            _options.AutoSelect = value;
            return this;
        }

        public IControlTableSelect<T> AutoFill(params ushort?[] minmaxwidth)
        {
        ...
        public IControlTableSelect<T> AddColumn(Expression<Func<T, object>> field, ushort width, Func<object, string> format = null, Alignment alignment = Alignment.Left, string? title = null, Alignment titlealignment = Alignment.Center, bool titlereplaceswidth = true, bool textcrop = false, ushort? maxslidinglines = null)
        {
            if (_options.AutoFill)
            {
        ...
        public IControlTableSelect<T> AddFormatType<T1>(Func<object,string> funcfomatType)
        {
            var type = typeof(T1);
        ...
        public IControlTableSelect<T> AddItem(T value, bool disable = false)
        {
            _options.Items.Add(new ItemTableRow<T>() { Value = value, Disabled = disable});
            return this;
        }

        public IControlTableSelect<T> AddItems(IEnumerable<T> values, bool disable = false)
        {
        ...
        public IControlTableSelect<T> AddItemsTo(AdderScope scope, params T[] values)
        {
            foreach (var item in values)
            {
                switch (scope)
        ...
        public IControlTableSelect<T> AutoFit(params ushort[] indexColumn)
        {
            if (_options.AutoFill)
            {
        ...
        public IControlTableSelect<T> Config(Action<IPromptConfig> context)
        {
            context?.Invoke(_options);
            return this;
        }

        public IControlTableSelect<T> HideSelectorRow(bool value = true)
        {
            _options.HideSelectorRow = value;
            return this;
        }

        public IControlTableSelect<T> HideHeaders(bool value = true)
        {
            _options.HideHeaders = value;
            return this;
        }

        public IControlTableSelect<T> Default(T value)
        {
            _options.DefaultValue = Optional<T>.Set(value);
            return this;
        }

        public IControlTableSelect<T> ColumnsNavigation(bool value = true)
        {
            _options.IsColumnsNavigation = value;
            return this;
        }

        public IControlTableSelect<T> Templates(Func<T, int, int, string> selectedTemplate = null, Func<T, int, int, string> finishTemplate = null)
        {
        ...
        public IControlTableSelect<T> ChangeDescription(Func<T, int, int, string> func = null)
        {
            _options.ChangeDescription = func;
            return this;
        }
        public IControlTableSelect<T> EqualItems(Func<T, T, bool> comparer)
        {
            _options.EqualItems = comparer;
            return this;
        }

        public IControlTableSelect<T> FilterByColumns(FilterMode filter, params ushort[] indexColumn)
        {
        ...
        public IControlTableSelect<T> Interaction<T1>(IEnumerable<T1> values, Action<IControlTableSelect<T>, T1> action)
        {
            foreach (var item in values)
            {
        ...
        public IControlTableSelect<T> Layout(TableLayout value)
        {
            _options.Layout = value;
            return this;
        }

        public IControlTableSelect<T> OrderBy(Expression<Func<T, object>> value)
        {
        ...
        public IControlTableSelect<T> OrderByDescending(Expression<Func<T, object>> value)
        {
        ...
        public IControlTableSelect<T> OverwriteDefaultFrom(string value, TimeSpan? timeout = null)
        {
        ...
        public IControlTableSelect<T> PageSize(int value)
        {
            if (value < 1)
            {
        ...
        public IControlTableSelect<T> SeparatorRows(bool value = true)
        {
        ...
        public IControlTableSelect<T> Title(string value, Alignment alignment = Alignment.Center, TableTitleMode titleMode = TableTitleMode.InLine)
        {
