...
{
    /// <summary>
    /// Represents a node of the tree exposed by <see cref="IMultiTreeControl{T}"/> while it is
    /// being constructed. Extends <see cref="ITreeNode{T}"/> so that chaining off a node returned
    /// by <see cref="AddLast"/>/<see cref="AddFirst"/> keeps access to the MultiTree-specific
    ...
    public interface IMultiTreeNode<T> : ITreeNode<T>
    {
        ...
        /// <see cref="IMultiTreeControl{T}.Default(System.Collections.Generic.IEnumerable{T}, bool)"/>
