...
#pragma warning disable IDE0290 // Use primary constructor
        public NodeTreeRemoteMultiSelectControl(IConsoleExtend console, PromptConfig promptConfig, BaseControlOptions baseControlOptions) : base(false, console, promptConfig, baseControlOptions)
        {
...
#pragma warning restore IDE0079
        ...
        private (string Key, bool IsLoadMore, bool IsLoad, bool IsFinished, Exception? Error, List<ItemNodeControl<T1>> Items) CreateEnqueueNewitems(ItemNodeControl<T1> parentnode, bool isLoadMore, bool isfinished, IEnumerable<(bool Checked, T1 NewItem)> result, Exception? err)
        {
            ...
            foreach ((bool Checked, T1 NewItem) in result)
            {
                pos++;
                bool first = false;
                bool last = false;
                ...
                string text = _textSelector!(NewItem);
                string fullpath = CreateFullPath(parentnode.UniqueId, text);
                string? extratxt = _extraInfoNode?.Invoke(NewItem) ?? null;
                bool allowsChildren = _predicateChildAllowed!(NewItem);
                NodeStatus sta = allowsChildren ? NodeStatus.NotLoad : NodeStatus.Done;

                newitems.Add(new ItemNodeControl<T1>(_uniqueexpression!(NewItem))
                {
                    IsExpanded = false,
                    Status = sta,
                    FirstItem = first,
