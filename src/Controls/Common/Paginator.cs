...
namespace PromptPlusLibrary.Controls.Common
{
    internal sealed class Paginator<T>
    {
        ...
        public byte PageSize => (byte)_userPageSize;
        ...
        public int TotalCountValid
        {
            get
            {
                ...
                {
                    if (_countValidator(item)) count++;
                }
                return count;
            }
        }
        public IEnumerable<T> AllItems() => _filteredItems;
        ...
        public bool End(IndexOption selectedIndexOption = IndexOption.LastItem)
        {
            ...
            int oldindex = SelectedIndex;
            int oldpage = SelectedPage;
            SelectedPage = (TotalCount - 1) / _userPageSize;
            SelectedIndex = 0;
        ...
        public void UpdateCollection(IEnumerable<T> items, Optional<T>? selected = null)
        {
            _items = [.. items];
            _firstDisabled = true;
            InitializeCollection();
        ...
        /// <summary>
        /// Changes the page size at runtime and recomputes pagination so that the currently
        /// selected item (when any) remains visible on the new page layout. Used by controls
        /// that adapt their page size to the available console height after a terminal resize.
        /// </summary>
        /// <param name="newPageSize">The new page size. Values less than 1 are coerced to 1.</param>
        public void UpdatePageSize(int newPageSize)
        {
            ...
            if (newPageSize == _userPageSize)
            {
                return;
            }
            // Preserve the global index of the currently selected item so it can be relocated
            // on the new page layout without losing the user's position.
            int previousGlobalIndex = SelectedIndex >= 0
                ? (_userPageSize * SelectedPage) + SelectedIndex
                : -1;

            _userPageSize = newPageSize;
            PageCount = _filteredItems.Length == 0 ? 0 : (_filteredItems.Length - 1) / _userPageSize + 1;

            if (previousGlobalIndex >= 0 && previousGlobalIndex < _filteredItems.Length)
            {
                SelectedIndex = previousGlobalIndex % _userPageSize;
                SelectedPage = previousGlobalIndex / _userPageSize;
            }
            else if (_filteredItems.Length > 0)
            {
                SelectedIndex = 0;
                SelectedPage = 0;
            }
        ...
        {
            if (!_firstDisabled && _filterMode != FilterMode.Disabled && _textSelector != null)
            {
        ...
        private void Initialize(Optional<T> defaultValue, Func<T, T, bool>? foundDefault)
        {
            ...
            {
                if (foundDefault.Invoke(_filteredItems[i], defaultValue.Value))
                {
        ...
        private (int index, int page) FindValidItem(int startIndex, bool forward)
        {
            ...
            while (true)
            {
                if (index < 0 || index >= _userPageSize)
                {
                    page = forward ? (page + 1) % PageCount : (page - 1 + PageCount) % PageCount;
                    index = forward ? 0 : _userPageSize - 1;
                }

                int globalIndex = (_userPageSize * page) + index;
                if (globalIndex < 0 || globalIndex >= _filteredItems.Length)
                {
                    break;
                }
