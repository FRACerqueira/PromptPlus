...
{
    internal sealed class NodeTreeMultiSelectControl<T> : BaseControlPrompt<T[]>, INodeTreeMultiSelectControl<T> where T : class, new()
    {
        ...
        public override bool TryResult(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    else if (!_onfilterOnlySelected && _localpaginator!.SelectedItem != null && !IsRoot(_localpaginator.SelectedItem) && _localpaginator.SelectedItem.CountChildren > 0 && "+-".Contains(keyinfo.KeyChar) && (keyinfo.Modifiers == ConsoleModifiers.Shift || keyinfo.Modifiers == ConsoleModifiers.None))
                    {
                        if (keyinfo.KeyChar == '+')
                        {
                            if (_localpaginator!.SelectedItem!.IsExpanded)
                    ...
                    else if (_localpaginator!.SelectedItem != null && _resultbuffer!.IsPrintable(keyinfo.KeyChar))
                    {
