...
namespace PromptPlusLibrary.Controls.FileMultiSelect
{
    internal sealed class FileMultiSelectControl : BaseControlPrompt<ItemFile[]>, IFileMultiSelectControl
    {
        ...
        private readonly List<ItemFile> _checkeditems = [];
...
#pragma warning restore IDE0079
        ...
        #region IFileMultiSelectControl

        public IFileMultiSelectControl HideFilesBySize(long minvalue, long maxvalue = long.MaxValue)
        {
        ...
        public IFileMultiSelectControl Options(Action<IControlOptions> options)
        {
        ...
        public override bool TryResult(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    {
                        _indexTooptip = 0;
                        ResultCtrl = new ResultPrompt<ItemFile[]>([], true);
                        break;
                    }
                    ...
                    else if (_modeView == ModeView.Select && keyinfo.IsPressEnterKey())
                    {
                        _indexTooptip = 0;
                        ...
                        _indexTooptip = 0;
                        _modeView = ModeView.Select;
                        ResultCtrl = new ResultPrompt<ItemFile[]>([.. _checkeditems], false);
                        break;
                    }
                    ...
                    else if (_modeView == ModeView.Select && keyinfo.IsPressSpaceKey() && _localpaginator!.SelectedItem != null && !_localpaginator.SelectedItem.IsDisabled)
                    {
                        ...
                        {
                            _resultbuffer!.LoadPrintable(_checkeditems.Select(x => x.Name!).Aggregate((x, y) => $"{x},{y}"));
                        }
                        int countselect = _checkeditems.Count;
                        if (countselect < _minSelect)
                        {
        ...
        public override bool FinishTemplate(BufferScreen screenBuffer)
        {
            ...
            screenBuffer.WriteLine(answer, _optStyles[FileStyles.Answer].Overflow(Overflow.Ellipsis));
        ...
        private void MarkAllItems(int index)
        {
            ...
            if (!mark)
            {
                chkindex = _checkeditems.FindIndex(x => x.FullPath == _items[index].Value.FullPath);
                _checkeditems.RemoveAt(chkindex);
            }
            else
            {
        ...
        private void WriteListSelect(BufferScreen screenBuffer)
        {
            ...
            foreach (ItemNodeControl<ItemFile> item in subset)
            {
                ...
                if (_localpaginator.TryGetSelected(out ItemNodeControl<ItemFile>? selectedItem) && EqualityComparer<ItemNodeControl<ItemFile>>.Default.Equals(item, selectedItem))
                {
                    ...
                    if (item.IsMarked)
                    {
                        screenBuffer.Write(ConfigPlus.GetSymbol(SymbolType.Selected), item.IsDisabled ? _optStyles[FileStyles.Disabled] : _optStyles[FileStyles.Selected]);
                    }
                    else
                    {
                        screenBuffer.Write(ConfigPlus.GetSymbol(SymbolType.NotSelect), item.IsDisabled ? _optStyles[FileStyles.Disabled] : _optStyles[FileStyles.Selected]);
                    }
                ...
                else
                {
                    ...
                    if (item.IsMarked)
                    {
                        screenBuffer.Write(ConfigPlus.GetSymbol(SymbolType.Selected), item.IsDisabled ? _optStyles[FileStyles.Disabled] : _optStyles[FileStyles.UnSelected]);
                    }
                    else
                    {
                        screenBuffer.Write(ConfigPlus.GetSymbol(SymbolType.NotSelect), item.IsDisabled ? _optStyles[FileStyles.Disabled] : _optStyles[FileStyles.UnSelected]);
                    }
                    screenBuffer.Write(CreateIndentation(item), _optStyles[FileStyles.Lines]);
                    if (checkroot)
                    {
            ...
            if (_localpaginator.PageCount > 1)
            {
                ...
                {
                    screenBuffer.Write(string.Format(Messages.TooltipCountCheck, _checkeditems.Count), _optStyles[FileStyles.TaggedInfo]);
                }
            ...
            else
            {
                if (!_hideCountSelected)
                {
                    screenBuffer.WriteLine(string.Format(Messages.TooltipCountCheck, _checkeditems.Count), _optStyles[FileStyles.TaggedInfo]);
                }
            }
        }
