...
{
    // TableMultiSelectControl, `Filter` mode (FilterMode != Disabled), covering both FilterTableMode
    ...
    // and `Select` mode are in TableMultiSelectControlTests.cs.
    public class TableMultiSelectControlFilterModeTests
    {
        private static VirtualTerminal MakeTerminal() => VirtualTerminal.Create(o => { o.SupportsUnicode = false; });

        private sealed record Person(string Name, int Age);

        private static ITableMultiSelectControl<Person> MakeAnswerFilterable(VirtualTerminal vt)
        ...
        private static ITableMultiSelectControl<Person> MakeColumnFiltersTable(VirtualTerminal vt)
            => new PromptPlusControls(vt, new PromptConfig()).TableMultiSelect<Person>("Choose")
        ...
        {
            // Bug found and fixed (same as TableSelectControl): Tab used to change the current column
