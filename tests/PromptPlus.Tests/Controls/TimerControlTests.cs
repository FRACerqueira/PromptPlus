...
{
    // TimerControl, the last "Live" control in this suite, deliberately saved for last because it
    ...
    public class TimerControlTests
    {
        ...
        private static ITimerControl MakeControl(VirtualTerminal vt) =>
        ...
        public void DisplayMode_Elapsed_shows_the_full_duration_instead_of_a_zero_countdown()
        {
            ...
            var control = MakeControl(vt).Duration(duration).DisplayMode(TimerDisplayMode.Elapsed);
        ...
        public async Task Spinner_and_ChangeDescription_are_observable_while_still_counting_down()
        {
            ...
            // asserted, per the user's explicit "don't require exact timing" guidance for Timer.
