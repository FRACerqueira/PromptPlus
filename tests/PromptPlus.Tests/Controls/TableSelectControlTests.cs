...
{
    // TableSelectControl, Select mode (global navigation across row/column/ViewOnly/disabled/predicate
    // outside of filter). Filter mode scenarios (Answer/ColumnFilters) are in
    // TableSelectControlFilterModeTests.cs. Checklist derived from reading the real
    // TryResult/BufferTemplate/FinishTemplate (TableSelectControl.cs) plus render probes.
    //
    ...
    //   cancellation (no key) always returns a default TableSelectResult<T> (null Value, Row/Col=0).
    ...
    public class TableSelectControlTests : IDisposable
    {
        ...
        public TableSelectControlTests() => FileHistory.FileSystem = _mock;
        ...
        private static ITableSelectControl<Person> MakeTable(VirtualTerminal vt)
            => new PromptPlusControls(vt, new PromptConfig()).TableSelect<Person>("Choose")
        ...
        public void Cancellation_without_a_real_keypress_returns_a_default_result()
        {
            ...
            // always returns TableSelectResult<T> default, it does not preserve wherever the cursor was.
            var vt = MakeTerminal();
            var control = MakeTable(vt);
            _ = vt.Keys.Enqueue(ConsoleKey.DownArrow);
        ...
        {
            var vt = MakeTerminal();
            var control = new PromptPlusControls(vt, new PromptConfig()).TableSelect<Person>("Choose")
                .AddColumn("Name", p => p.Name)
