...
{
    // TableSelectControl, `Filter` mode (FilterMode != Disabled), covering both FilterTableMode values:
    ...
    // TableSelectControlTests.cs.
    public class TableSelectControlFilterModeTests
    {
        private static VirtualTerminal MakeTerminal() => VirtualTerminal.Create(o => { o.SupportsUnicode = false; });

        private sealed record Person(string Name, int Age);

        private static ITableSelectControl<Person> MakeAnswerFilterable(VirtualTerminal vt)
        ...
        private static ITableSelectControl<Person> MakeColumnFiltersTable(VirtualTerminal vt)
            => new PromptPlusControls(vt, new PromptConfig()).TableSelect<Person>("Choose")
