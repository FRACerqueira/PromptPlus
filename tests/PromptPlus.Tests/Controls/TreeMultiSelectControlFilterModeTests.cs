...
{
    // TreeMultiSelectControl, `Filter` mode (FilterMode != Disabled). Globals and `Select` mode are in
    // TreeMultiSelectControlTests.cs.
    //
    ...
    public class TreeMultiSelectControlFilterModeTests
    {
        private static VirtualTerminal MakeTerminal() => VirtualTerminal.Create(o => { o.SupportsUnicode = false; });

        private static ITreeMultiSelectControl<string> MakeFilterableTree(VirtualTerminal vt)
        {
