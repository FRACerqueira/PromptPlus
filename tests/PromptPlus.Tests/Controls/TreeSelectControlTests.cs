...
{
    // TreeSelectControl, `Select` mode (global behavior + expand/collapse/Tab-drilldown/ViewOnly/
    // SelectLeafOnly/predicate outside of filter). Filter mode scenarios are in
    // TreeSelectControlFilterModeTests.cs. Checklist derived from reading the real
    // TryResult/BufferTemplate/InitControl (TreeSelectControl.cs) plus render probes.
    //
    ...
    public class TreeSelectControlTests : IDisposable
    {
        ...
        public TreeSelectControlTests() => FileHistory.FileSystem = _mock;
        ...
        //    A2
        //  Berry
        private static ITreeSelectControl<string> MakeTree(VirtualTerminal vt)
        {
        ...
        {
            var vt = MakeTerminal();
            var control = new PromptPlusControls(vt, new PromptConfig()).TreeSelect<string>("Choose")
                .Root("Root")
                .TextSelector(x => x)
                .DefaultMatchBy((a, b) => a == b);
