# Migration v5.x → v6.x: TableSelect and TableMultiSelect

> Back to [Migration Overview](../../migration-v5-to-v6.md)

## Factory methods unchanged

`TableSelect<T>()` and `TableMultiSelect<T>()` keep their v5.x names in v6.x — only the methods
below changed.

## Renamed public methods

`TableMultiSelect<T>` **renamed** its confirmation validator. `TableSelect<T>` (single-selection) keeps
`PredicateSelected`.

| Control | v5.x member | v6.x member |
|---|---|---|
| TableMultiSelect | `PredicateSelected(Func<T, bool>)` | `PredicateChecked(Func<T, bool>)` |
| TableMultiSelect | `PredicateSelected(Func<T, (bool, string?)>)` | `PredicateChecked(Func<T, (bool, string?)>)` |

> The async overloads follow the same name: `TableMultiSelect` uses `PredicateCheckedAsync`; `TableSelect` uses
...
### 9. `TableSelect<T>` return type changed (single-select)
...
// v6.x — TableSelect<T>.Run() → ResultPrompt<TableSelectResult<T>>
var result = PromptPlus.Controls.TableSelect<Product>("Products:")
...
> `TableMultiSelect<T>.Run()` still returns `ResultPrompt<T[]>` (read via `.Content`).

---

## What's new in v6.x

### `HorizontalScroll` (TableSelect and TableMultiSelect)
```csharp
PromptPlus.Controls.TableSelect<Product>("Products:")
...
### TableMultiSelect — history, view-only and async variants
```csharp
...
New on `TableSelect`/`TableMultiSelect`: `DefaultMatchBy` · `ViewOnly` · `HorizontalScroll` · `ChangeDescriptionAsync` · `TextSelectorAsync` · `InteractionAsync`; `TableSelect` adds `PredicateSelectedAsync` (x2) and `TableMultiSelect` adds `PredicateCheckedAsync` (x2); plus `EnableHistory` / `UseDefaultHistory` on `TableMultiSelect`.

---

## Full API reference

### TableSelect\<T\> — v5.x vs v6.x

| Method | v5.x | v6.x | Change |
|---|---|---|---|
| Factory | `TableSelect<T>()` | `TableSelect<T>()` | Unchanged |
...
| Method | v5.x | v6.x | Change |
|---|---|---|---|
| Factory | `TableMultiSelect<T>()` | `TableMultiSelect<T>()` | Unchanged |
