# Migration v5.x → v6.x: TreeSelect and TreeMultiSelect
...
| v5.x | v6.x |
|---|---|
| `NodeTreeSelect<T>()` | `TreeSelect<T>()` |
| `NodeTreeMultiSelect<T>()` | `TreeMultiSelect<T>()` |

## Renamed public methods

Beyond the factory renames above, `TreeMultiSelect<T>` **renamed** its confirmation validator. `TreeSelect<T>`
(single-selection) keeps `PredicateSelected`.

| Control | v5.x member | v6.x member |
|---|---|---|
| TreeMultiSelect | `PredicateSelected(Func<T, bool>)` | `PredicateChecked(Func<T, bool>)` |
| TreeMultiSelect | `PredicateSelected(Func<T, (bool, string?)>)` | `PredicateChecked(Func<T, (bool, string?)>)` |
...
var control = PromptPlus.Controls.TreeSelect<Department>("Department:")
    .TextSelector(d => d.Name)
...
// v6.x
.Styles(TreeSelectStyles.Selected, style)        // TreeMultiSelectStyles for TreeMultiSelect
```
...
var control = PromptPlus.Controls.TreeSelect<Category>("Category:")
...
New on `TreeSelect`/`TreeMultiSelect`: `ViewOnly` · `Filter` · `ShowFullPath` · `DefaultMatchBy` · `Default` / `EnableHistory` · `AddAfter` / `AddBefore` · `ChangeDescriptionAsync` · `ExtraInfoAsync` · `InteractionAsync`. `TreeSelect` adds `SelectLeafOnly` and `PredicateSelectedAsync` (x2); `TreeMultiSelect` adds `CheckLeafOnly`, `CascadeCheck`, `RecursiveMarkWithCtrlSpace`, `PredicateCheckedAsync` (x2), plus the node-level `disable` / `check` parameters on `Root`/`Add*`.

```csharp
PromptPlus.Controls.TreeMultiSelect<Category>("Categories:")
    .TextSelector(c => c.Name)
...
### TreeSelect\<T\> — v5.x vs v6.x

| Method | v5.x (NodeTreeSelect) | v6.x (TreeSelect) | Change |
|---|---|---|---|
| Factory | `NodeTreeSelect<T>()` | `TreeSelect<T>()` | Renamed |
| `AddRootNode(T, string nodeseparator)` | ✅ | ❌ | → `Root(T)` + `PathSeparator(char)` |
...
| `Styles(NodeTreeStyles, Style)` | ✅ | ❌ | → `Styles(TreeSelectStyles, Style)` |
...
### TreeMultiSelect\<T\> — v5.x vs v6.x

| Method | v5.x (NodeTreeMultiSelect) | v6.x (TreeMultiSelect) | Change |
|---|---|---|---|
