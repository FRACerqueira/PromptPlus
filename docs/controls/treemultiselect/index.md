...
[← Back to Home](../../../README.md) • **Next:** [TreeMultiSelect — Methods →](methods.md)
...
aggregate state of their descendants. You build the structure exactly like [`TreeSelect<T>`](../treeselect/index.md)
...
> 🔘 Only need to pick **one** node? Use the [**TreeSelect**](../treeselect/index.md) control — same tree model,
> single selection.
...
| [Styles](styles.md) | The `TreeMultiSelectStyles` regions and how to recolor them |

---

## When to use it

| Use `TreeMultiSelect<T>` when… | Consider instead… |
|---|---|
| The data is hierarchical and the user checks several nodes | — |
| The user picks a single node | [TreeSelect](../treeselect/index.md) |
...
- `TreeMultiSelect<string>("Check folders")` creates a tree of strings. The type argument `T` is the node type.
...
You add nodes explicitly, exactly as in [`TreeSelect<T>`](../treeselect/index.md). First-level nodes are children
...
- [TreeSelect](../treeselect/index.md) — single-choice sibling
- [MultiSelect](../multiselect/index.md) · [TableSelect](../tableselect/index.md) — flat and tabular pickers
