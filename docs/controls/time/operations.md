...
[← Back to Home](../../../README.md) • **Next:** [Time — Styles →](styles.md)

---

How the `Time` control behaves while it runs: the required duration, count-down vs. count-up display,
...
The `Time` control is non-interactive: there is nothing to type or select. The only user action is
...
```csharp
PromptPlus.Controls.Time("Starting in").Duration(3).Run();                     // 3 seconds
...
{
    var result = PromptPlus.Controls.Time("Cancelable countdown")
        .Duration(10)
        .Run(sw.Token);
...
- **Not a picker.** `Time` waits and displays a clock; it does not collect a time value. To *enter* a
...
- **A blank prompt is fine:** `PromptPlus.Controls.Time().Duration(5)...` renders just the countdown.
