...
[← Back to Home](../../../README.md) • **Next:** [Time — Methods →](methods.md)
...
The `Time` control is **not** a time picker. It is a visible, cancellable wait: a "starting in 3…"
...
| [Styles](styles.md) | The `TimeStyles` regions and how to recolor them |

---

## When to use it

| Use `Time` when… | Consider instead… |
|---|---|
...
- `Time("Starting in")` creates the control. The first argument is the **prompt**; an optional second
...
var result = PromptPlus.Controls
    ...
    .DisplayMode(TimeDisplayMode.Countdown)         // count down (default)
...
`Time` returns `ResultPrompt<TimeSpan>`.
...
```csharp
var result = PromptPlus.Controls.Time("Starting in").Duration(3).Run();
