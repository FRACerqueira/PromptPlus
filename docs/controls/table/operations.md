...
[← Back to Home](../../../README.md) • **Next:** [Table — Styles →](styles.md)

---

How the `Table<T>` control behaves while it is running: keyboard, columns, filtering, horizontal
...
- **Configuration is required.** `Run` throws `ValidationException` unless at least one column **and** one
  row have been added.
- **Aborted results** carry `.Content == default` (a `TableResult<T>` with `default(T)`). Always branch on
  `IsAborted`.
...
- [MultiTable → Operations](../multitable/operations.md) — checkbox behavior for the multiple-row sibling
