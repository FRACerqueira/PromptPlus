...
[← Back to Home](../../../README.md) • **Next:** [Table — Operations →](operations.md)

---

Every fluent method on `ITableControl<T>`. Each returns the same control instance, so calls chain
in any order. Call [`Run`](#run) last.

> The factory is `PromptPlus.Controls.Table<T>(string prompt = "", string? description = null)`,
> which returns `ITableControl<T>`.
...
```csharp
PromptPlus.Controls.Table<Product>("Select a product")
...
```csharp
PromptPlus.Controls.Table<Product>("Product")
...
```csharp
PromptPlus.Controls.Table<Product>("Select an available product")
    .AddColumn("Name",      x => x.Name)
...
```csharp
PromptPlus.Controls.Table<Product>("Search product")
    .AddColumn("Name",     x => x.Name,     isFilterable: true)
...
```csharp
PromptPlus.Controls.Table<Product>("Product").AddColumn("Name", x => x.Name).AddItems(products).PageSize(8).Run();
```
...
```csharp
PromptPlus.Controls.Table<Employee>("Employee")
...
```csharp
PromptPlus.Controls.Table<Product>("Product catalogue (view only)", "Press Esc or Enter to exit")
    .AddColumn("Name",  x => x.Name)
    .AddColumn("Price", x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
    .AddItems(products)
    .PageSize(4)
...
[`ResultPrompt<TableResult<T>>`](../../architecture.md#resultpromptt); the `TableResult<T>` exposes
...
- [Styles](styles.md) — the `TableStyles` regions
- [Index](index.md) — overview and method map
- [MultiTable → Methods](../multitable/methods.md) — the multiple-row sibling
