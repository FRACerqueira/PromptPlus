...
[← Back to Home](../../../README.md) • **Next:** [TreeSelect — Operations →](operations.md)
...
> The factory is `PromptPlus.Controls.TreeSelect<T>(string prompt = "", string? description = null)`,
> which returns `ITreeSelectControl<T>`.
...
```csharp
ITreeSelectControl<T> Root(T value)
```
...
```csharp
ITreeSelectControl<T> Interaction<T1>(IEnumerable<T1> items, Action<T1, ITreeSelectControl<T>> interactionAction)
```
...
```csharp
ITreeSelectControl<T> InteractionAsync<T1>(IEnumerable<T1> items, Func<T1, ITreeSelectControl<T>, Task> interactionAction)
```
...
```csharp
ITreeSelectControl<T> TextSelector(Func<T, string> selector)
```
...
```csharp
ITreeSelectControl<T> ExtraInfo(Func<T, string?> extraInfoNode)
```
...
```csharp
ITreeSelectControl<T> ExtraInfoAsync(Func<T, Task<string?>> extraInfoNode)
```
...
```csharp
ITreeSelectControl<T> PathSeparator(char value)
```
...
```csharp
ITreeSelectControl<T> ShowFullPath(bool value = true)
```
...
```csharp
ITreeSelectControl<T> PageSize(byte value)
```
...
```csharp
ITreeSelectControl<T> Filter(FilterMode value)
```
...
```csharp
ITreeSelectControl<T> SelectLeafOnly(bool value = true)
```
...
```csharp
ITreeSelectControl<T> PredicateSelected(Func<T, bool> validselect)
...
```csharp
ITreeSelectControl<T> PredicateSelectedAsync(Func<T, Task<bool>> validselect)
...
```csharp
ITreeSelectControl<T> Default(T value, bool useDefaultHistory = true)
```
...
```csharp
ITreeSelectControl<T> DefaultMatchBy(Func<T, T, bool> comparer)
```
...
```csharp
ITreeSelectControl<T> ChangeDescription(Func<T, string> value)
```
...
```csharp
ITreeSelectControl<T> ChangeDescriptionAsync(Func<T, Task<string>> value)
```
...
```csharp
ITreeSelectControl<T> EnableHistory(string filename, Action<IHistoryOptions>? options = null)
```
...
```csharp
ITreeSelectControl<T> Styles(TreeSelectStyles styleType, Style style)
```
...
```csharp
ITreeSelectControl<T> Options(Action<IControlOptions> options)
```
...
- [Styles](styles.md) — the `TreeSelectStyles` regions
- [Index](index.md) — overview and method map
- [TreeMultiSelect → Methods](../treemultiselect/methods.md) — the multiple-choice sibling
