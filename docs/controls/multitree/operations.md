...
How the `MultiTree<T>` control is built and how it behaves while running: the tree model, keyboard,
...
The hierarchy is built explicitly, in code, before `Run` — identical to [`Tree<T>`](../tree/operations.md#building-the-tree-model).
...
```csharp
var tree = PromptPlus.Controls.MultiTree<Node>("Check items")
    .Root(company)
