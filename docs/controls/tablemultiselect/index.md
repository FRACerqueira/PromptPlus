...
[← Back to Home](../../../README.md) • **Next:** [TableMultiSelect — Methods →](methods.md)
...
`TableMultiSelect<T>` is the multiple-selection sibling of [`TableSelect<T>`](../tableselect/index.md). It renders any
...
> 🔘 Only need **one** row? Use the [**TableSelect**](../tableselect/index.md) control — same grid model, single choice.
...
| [Styles](styles.md) | The `TableMultiSelectStyles` regions and how to recolor them |

---

## When to use it

| Use `TableMultiSelect<T>` when… | Consider instead… |
|---|---|
| The data is tabular and the user checks several rows | — |
| The user picks exactly one row | [TableSelect](../tableselect/index.md) |
...
var result = PromptPlus.Controls
    .TableMultiSelect<Product>("Select products", "Check all desired items")
...
- `TableMultiSelect<Product>(...)` creates the grid. The type argument `T` is the **row** type.
...
var result = PromptPlus.Controls
    .TableMultiSelect<Product>("Select 2 to 4 products", "Available items pre-checked")
    .AddColumn("Name",      x => x.Name)
    .AddColumn("Category",  x => x.Category)
...
`TableMultiSelect<T>` returns `ResultPrompt<T[]>` — an array of the checked rows.
...
[`TableSelect<T>`](../tableselect/index.md), which returns a single `TableSelectResult<T>`).
...
- [TableSelect](../tableselect/index.md) — single-row sibling
- [Select](../select/index.md) · [TreeSelect](../treeselect/index.md) — single-column and hierarchical pickers
