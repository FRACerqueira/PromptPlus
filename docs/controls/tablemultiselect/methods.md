...
[← Back to Home](../../../README.md) • **Next:** [TableMultiSelect — Operations →](operations.md)
...
> The factory is `PromptPlus.Controls.TableMultiSelect<T>(string prompt = "", string? description = null)`,
> which returns `ITableMultiSelectControl<T>`.
...
```csharp
ITableMultiSelectControl<T> Interaction<T1>(IEnumerable<T1> items, Action<T1, ITableMultiSelectControl<T>> interactionAction)
```
...
```csharp
ITableMultiSelectControl<T> InteractionAsync<T1>(IEnumerable<T1> items, Func<T1, ITableMultiSelectControl<T>, Task> interactionAction)
```
...
- [Styles](styles.md) — the `TableMultiSelectStyles` regions
- [Index](index.md) — overview and method map
- [TableSelect → Methods](../tableselect/methods.md) — the single-row sibling
