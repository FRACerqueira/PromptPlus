...
[← Back to Home](../../../README.md) • **Next:** [TableMultiSelect — Styles →](styles.md)
...
- **No `.Value` wrapper.** Iterating `.Content` yields each `T` directly, unlike
  [`Table<T>`](../tableselect/index.md) which returns a single `TableSelectResult<T>`.
...
- [TableSelect → Operations](../tableselect/operations.md) — the single-row sibling
