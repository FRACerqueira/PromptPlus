...
[← Back to Home](../../../README.md) • **Next:** [TableSelect — Methods →](methods.md)

---

> A paginated, filterable grid with named columns where the user picks **one row** and confirms with **Enter**.

`TableSelect<T>` renders any collection as a bordered table. You declare the columns (header, value selector,
...
> ☑️ Need to pick **several** rows at once? Use the [**TableMultiSelect**](../tablemultiselect/index.md)
...
| [Styles](styles.md) | The `TableSelectStyles` regions and how to recolor them |
...
| The user may check several rows | [TableMultiSelect](../tablemultiselect/index.md) |
...
- `TableSelect<Product>("Select a product")` creates the grid. The type argument `T` is the **row** type.
...
- `.Run()` renders the table and blocks until **Enter** (confirm) or **Esc** (abort), returning
  [`ResultPrompt<TableSelectResult<T>>`](../../architecture.md#resultpromptt).
...
`TableSelect<T>` returns `ResultPrompt<TableSelectResult<T>>`. The `TableSelectResult<T>` carries the confirmed row and
its table coordinates.

| Member | Meaning |
|---|---|
| `.Content` | The [`TableSelectResult<T>`](methods.md#run) (default when aborted) |
...
- [TableMultiSelect](../tablemultiselect/index.md) — multiple-row sibling
