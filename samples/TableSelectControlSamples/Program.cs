...
namespace TableSelectControlSamples;
...
internal static class Program
{
    ...
    static void Main()
    {
        ...
        var r1 = PromptPlus.Controls.TableSelect<Product>("Select a product", "All column features in one table")
        ...
        var r2 = PromptPlus.Controls.TableSelect<Product>("Select an available product")
        ...
        var r3a = PromptPlus.Controls.TableSelect<Product>("Select product from Electronics/Peripherals")
            .AddColumn("Name",     x => x.Name)
            .AddColumn("Category", x => x.Category)
            .AddColumn("Price",    x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
            .AddItems(s_products)
        ...
        var r3b = PromptPlus.Controls.TableSelect<Product>("Select product with stock > 100")
            .AddColumn("Name",  x => x.Name)
        ...
        PromptPlus.Controls.TableSelect<Product>("Search product - Contains / Answer")
            .AddColumn("Name",     x => x.Name,     isFilterable: true)
        ...
        PromptPlus.Controls.TableSelect<Product>("Search product - StartsWith / ColumnFilters")
            .AddColumn("Name",     x => x.Name,     isFilterable: true)
            .AddColumn("Category", x => x.Category, isFilterable: true)
            .AddColumn("Origin",   x => x.Origin,   isFilterable: true)
            .AddItems(s_products)
        ...
        PromptPlus.Controls.TableSelect<Product>("Search product - Contains / ColumnFilters")
            .AddColumn("Name",     x => x.Name,     isFilterable: true)
            .AddColumn("Category", x => x.Category, isFilterable: true)
            .AddColumn("Origin",   x => x.Origin,   isFilterable: true)
        ...
        var r7 = PromptPlus.Controls.TableSelect<Product>("Select product (async description + answer)")
            .AddColumn("Name",  x => x.Name)
            .AddColumn("Price", x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
        ...
        var r8 = PromptPlus.Controls.TableSelect<Product>("Select product (loaded via interaction)")
            .AddColumn("Id",    x => x.Id,    width: 4, alignment: ColumnAlignment.Right)
            .AddColumn("Name",  x => x.Name)
            .AddColumn("Price", x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
        ...
        var r9 = PromptPlus.Controls.TableSelect<Product>("Select product (history enabled - run twice!)")
            .AddColumn("Id",   x => x.Id,   width: 4, alignment: ColumnAlignment.Right)
            .AddColumn("Name", x => x.Name, isFilterable: true)
            .AddItems(s_products)
        ...
        var r12 = PromptPlus.Controls.TableSelect<Product>("Product catalogue (view only)", "Press Esc or Enter to exit")
            .AddColumn("Id",       x => x.Id,       width: 4,  alignment: ColumnAlignment.Right)
            .AddColumn("Name",     x => x.Name)
            .AddColumn("Category", x => x.Category)
            .AddColumn("Price",    x => x.Price,    v => $"${v:N2}", alignment: ColumnAlignment.Right)
        ...
        // --------------------------------------------------------------------
        // 13 - Custom Styles (TableSelectStyles)
        ...
        var r13 = PromptPlus.Controls.TableSelect<Product>("Select product (custom styles)")
            .AddColumn("Name",     x => x.Name)
            .AddColumn("Category", x => x.Category)
            .AddColumn("Price",    x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
            .Interaction(s_products, (p, ctrl) => ctrl.AddItem(p, disable: !p.Available))
    ...
    {
        PromptPlus.Controls.TableSelect<Product>($"LayoutMode.{mode}")
            .LayoutMode(mode)
            .AddColumn("Name",     x => x.Name)
            .AddColumn("Category", x => x.Category)
            .AddColumn("Price",    x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
    ...
    {
        PromptPlus.Controls.TableSelect<Product>($"HideElements: {label}")
            .HideElements(hide)
            .AddColumn("Name",     x => x.Name)
            .AddColumn("Category", x => x.Category)
            .AddColumn("Price",    x => x.Price, v => $"${v:N2}", alignment: ColumnAlignment.Right)
    ...
    private static ITableSelectControl<Employee> BuildEmployeeTable(string prompt, HorizontalScrollMode scroll)
    {
        return PromptPlus.Controls.TableSelect<Employee>(prompt, "12 columns - use ? ? to scroll columns")
    ...
    private static void PrintResult<T>(ResultPrompt<TableSelectResult<T>> result)
    {
        if (result.IsAborted)
        {
