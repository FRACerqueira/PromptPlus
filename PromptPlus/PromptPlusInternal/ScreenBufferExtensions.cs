...
namespace PromptPlusInternal
{
    internal static class ScreenBufferExtensions
    {
        ...
        public static void WriteLineStandardHotKeys(this ScreenBuffer screenBuffer, bool overpipeline, bool enabledabortkey, bool enabledabortAllpipes, bool hidedescription)
        {
            ...
            if (enabledabortkey)
            {
                if (overpipeline)
                {
                    if (!hidedescription)
                    {
                        screenBuffer.Write(string.Format(Messages.ShowStandardHotKeysWithPipelineDesc, TooltipKeyPress, ToggleVisibleDescription, ResumePipesKeyPress, enabledabortAllpipes ? Messages.EscCancelWithPipeline : Messages.EscCancelWithPipeNotAll), ColorSchema.Hint, _consoleDriver.BackgroundColor);
                    }
                    else
                    {
                        screenBuffer.Write(string.Format(Messages.ShowStandardHotKeysWithPipeline, TooltipKeyPress, ResumePipesKeyPress, enabledabortAllpipes ? Messages.EscCancelWithPipeline : Messages.EscCancelWithPipeNotAll), ColorSchema.Hint, _consoleDriver.BackgroundColor);
                    }
                }
                else
                {
        ...
        {
            screenBuffer.Write(text, _consoleDriver.ForegroundColor, _consoleDriver.BackgroundColor);
        }
        ...
        public static void Write(this ScreenBuffer screenBuffer, string text, ConsoleColor color, ConsoleColor? colorbg = null)
        {
            ...
            screenBuffer.Last().Add(new TextInfo(text, color, colorbg ?? _consoleDriver.BackgroundColor));
        }

        public static void WriteLine(this ScreenBuffer screenBuffer)
        {
            screenBuffer.Add(new List<TextInfo>() { new TextInfo("", _consoleDriver.ForegroundColor, _consoleDriver.BackgroundColor) });
        }
        ...
        {
            screenBuffer.WriteLine();
            screenBuffer.Last().Add(new TextInfo(text ?? string.Empty, color ?? _consoleDriver.ForegroundColor, colorbg ?? _consoleDriver.BackgroundColor));
        }
